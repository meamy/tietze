...
test58 = TestCase (assertEqual "Unknown operators rejected for 3 qubit circuits (1/2)."
                               (Left "Unknown three qubit operator: MadeUp")
                               (interpret3QubitCliffordDTofGate word))
    where word = "X[0].X[1].Z[0].Z[1].MadeUp.Z[0].Z[1].X[0].X[1]"
...
test62 = TestCase (assertEqual "Can interpret CCZ as a 3 qubit circuit."
                               (Right gateCCZ)
                               (interpret3QubitCliffordDTofGate "CCZ"))

-----------------------------------------------------------------------------------------
-- Products Mod P Semantics Parsing.

test63 = TestCase (assertEqual "tokenizeByIntIncl works on valid inputs (1/2)."
                               (Just ("10", "abc sdsad  ") :: Maybe (String, String))
                               (tokenizeByIntIncl show "10abc sdsad  "))

test64 = TestCase (assertEqual "tokenizeByIntIncl works on valid inputs (2/2)."
                               (Just ("-72", " a45c ad  ") :: Maybe (String, String))
                               (tokenizeByIntIncl show "-72 a45c ad  "))

test65 = TestCase (assertEqual "tokenizeByIntIncl detects invalid inputs."
                               (Nothing :: Maybe (String, String))
                               (tokenizeByIntIncl show "abc sdsad  "))

test66 = TestCase (assertEqual "Rejects an empty line as MultProductModP."
                               (Left "Unable to parse product components")
                               (interpretMultProductModP [] ""))

test67 = TestCase (assertEqual "Can parse an empty MultProductModP."
                               (Right (promoteToProduct list))
                               (interpretMultProductModP [] "()"))
    where list = [] :: [ArithModP MultInt]

test68 = TestCase (assertEqual "Can parse a 1-component MultProductModP (1/2)."
                               (Right (promoteToProduct [x1]))
                               (interpretMultProductModP [5] "(9)"))
    where x1 = fromJust $ inclusionModP (MultInt 9) 5

test69 = TestCase (assertEqual "Can parse a 1-component MultProductModP (2/2)."
                               (Right (promoteToProduct [x1]))
                               (interpretMultProductModP [0] "(-5)"))
    where x1 = fromJust $ inclusionModP (MultInt (-5)) 0

test70 = TestCase (assertEqual "Can parse a 2-component MultProductModP (1/2)."
                               (Right (promoteToProduct [x1, x2]))
                               (interpretMultProductModP [5, 0] "(9, 72)"))
    where x1 = fromJust $ inclusionModP (MultInt 9) 5
          x2 = fromJust $ inclusionModP (MultInt 72) 0

test71 = TestCase (assertEqual "Can parse a 2-component MultProductModP (2/2)."
                               (Right (promoteToProduct [x1, x2]))
                               (interpretMultProductModP [2, 99] "(-56,0)"))
    where x1 = fromJust $ inclusionModP (MultInt (-56)) 2
          x2 = fromJust $ inclusionModP (MultInt 0) 99

test72 = TestCase (assertEqual "Can parse a 3-component MultProductModP."
...
test73 = TestCase (assertEqual "Can parse a 3-component AddProductModP."
                               (Right (promoteToProduct [x1, x2, x3]))
...
test74 = TestCase (assertEqual "Can detect a bad tuple for MultProductModP."
                               (Left "Unable to parse product components")
                               (interpretMultProductModP [] "sadsadsa"))

test75 = TestCase (assertEqual "Can detect a bad EOL for MultProductModP."
                               (Left "Unexpected characters after product")
                               (interpretAddProductModP [0, 0, 0] "(9, 72, 4) xyz"))

test76 = TestCase (assertEqual "Can localize bad element in MultProductModP (1/2)."
                               (Left "Could not interpret 1-th component")
                               (interpretAddProductModP [(-1), 0, 0] "(9, 72, 4)"))

test77 = TestCase (assertEqual "Can localize bad element in MultProductModP (2/2)."
...
tests = hUnitTestToTests $ TestList [TestLabel "ParseSem_2Qubit_ID" test1,
                                     ...
                                     TestLabel "ParseSem_3Qubit_CCZ" test62,
                                     TestLabel "tokenizeByIntIncl_Valid_1" test63,
                                     TestLabel "tokenizeByIntIncl_Valid_2" test64,
                                     TestLabel "tokenizeByIntIncl_Invalid" test65,
                                     TestLabel "interpretMultProductModP_Empty" test66,
                                     TestLabel "interpretMultProductModP_OProd" test67,
                                     TestLabel "interpretMultProductModP_1Prod_1" test68,
                                     TestLabel "interpretMultProductModP_1Prod_2" test69,
                                     TestLabel "interpretMultProductModP_2Prod_1" test70,
                                     TestLabel "interpretMultProductModP_2Prod_2" test71,
                                     TestLabel "interpretMultProductModP_3Prod" test72,
                                     TestLabel "interpretAddProductModP_3Prod" test73,
                                     TestLabel "interUpretMultProductModP_BadProd" test74,
                                     TestLabel "interpretMultProductModP_BadEOL" test75,
                                     TestLabel "interpretMultProductModP_BadComp_1" test76,
                                     TestLabel "interpretMultProductModP_BadComp_2" test77]
