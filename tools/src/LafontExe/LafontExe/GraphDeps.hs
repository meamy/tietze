...
generateDepGraph :: Style -> [Named AbsDerivation] -> String
generateDepGraph sty named =
    case derivationsToDotFile abs of
...
processDerivationFiles :: Handle -> Style -> [String] -> RuleDict -> [String] -> IO ()
processDerivationFiles hdl sty fnames rules gens = do
    ...
    case readResult of
        ...
        Right prederivs       -> case processPreDerivations prederivs rules gens of
            ...
            NamedDerivs derivs    -> hPutStr hdl $ generateDepGraph sty derivs

-- | See graphDeps. Requires that both files exist, whereas graphDeps does not impose
-- this assumption.
graphDepsImpl :: Handle -> Style -> String -> [String] -> [String] -> IO ()
graphDepsImpl hdl sty genFname relFnames derivFnames = do
    ...
    case readGeneratorsAndRules genFile relFiles of
        ...
        GenRulePair gens rules -> processDerivationFiles hdl sty derivFnames rules gens
...
-- number.
graphDeps :: Handle -> Style -> String -> NonEmpty String -> [String] -> IO ()
graphDeps hdl sty genFname relFnames derivFnames = do
    res <- doFilesExist $ genFnames' ++ relFnames' ++ derivFnames
    case res of
        Just name -> hPutStr hdl $ "File does not exist: " ++ name ++ "\n"
        Nothing   -> graphDepsImpl hdl sty genFname relFnames' derivFnames
